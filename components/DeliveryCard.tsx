      ...
      <View style={fullWidth && { height: "100%" }}>
        ...
        <View style={tw("items-start p-5 -mt-3")}>
          <View style={tw("mx-auto")}>
            ...
            <Text style={tw("text-white text-center text-lg font-bold mb-3")}>
              Expected Delivery:{" "}
        ...
        <View style={tw("p-5")}>
          {order.trackingItems.items.map((i) => (
            <View
              key={i.item_id}
              style={tw("flex-row justify-between items-center")}
            >
              <Text style={tw("text-sm italic text-white")}>{i.name}</Text>
              <Text style={tw("text-white text-lg")}>x{i.quantity}</Text>
            </View>
          ))}
        </View>
